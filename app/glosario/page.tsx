...
export default function GlossaryPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="app-eyebrow">Términos clave</p>
        <h1 className="font-display text-3xl font-bold">Glosario técnico balear</h1>
      </div>
      <section className="app-card flex flex-wrap gap-3 rounded-xl p-4">
        <div className="relative min-w-72 flex-1">
          ...
          <input
            className="app-control focus-ring w-full rounded-xl py-2 pl-9 pr-3 text-sm"
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => (
          <article key={item.id} className="app-card-interactive animate-rise rounded-xl p-5">
            <h2 className="font-display text-xl font-bold">{item.term}</h2>
            <p className="app-muted mt-2 text-sm leading-6">{item.definition}</p>
            <p className="app-soft mt-3 rounded-xl p-3 text-sm">{item.easyDefinition}</p>
            {item.balearContext ? (
